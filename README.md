...
<img src="https://img.shields.io/badge/version-0.2.2-fix1-ffaf50?style=for-the-badge" alt="Version"/>
</div>
<div><b>简体中文</b></div>
</div>
...
| 类名 | 中文名 | 费用 | 类型 | 稀有度 | 目标 | 效果 |
|------|------|------|------|------|------|------|
| `EventAngelsDescent` | 天使降临 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>所有单位回复{Heal:diff()}点生命。 |
| `EventCrowdedPassage` | 拥挤通道 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>所有玩家获得{Buffer:diff()}层缓冲和{Vulnerable:diff()}层易伤。 |
| `EventDeusExMachina` | 天降神兵 | `0` | `技能` | `稀有` | `自己` | 事件<br>选择1张除天降神兵外的事件牌，发动2次。 |
| `EventEquality` | 人人平等 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>将所有友方的生命值设为1，并获得{Block:diff()}点格挡。 |
| `EventFightFun` | 战斗，爽！ | `0` | `技能` | `稀有` | `所有友方` | 事件<br>所有玩家获得1张巨石。 |
| `EventFoodSafety` | 食品安全 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>所有单位获得{Poison:diff()}层中毒。 |
| `EventFoodSafetyDoom` | 食品安全 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>所有单位获得{DoomPower:diff()}层灾厄。 |
| `EventGiftFromSky` | 天降之物 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>所有玩家抽{Cards:diff()}张牌，并获得{StarLight:diff()}点星光。 |
| `EventHandErase` | 手牌抹除 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>所有玩家弃置手牌最右侧的1张牌。 |
| `EventPlayerRepresentative` | 玩家代表 | `0` | `技能` | `稀有` | `自己` | 事件<br>只有你进行判定。<br>若结果为1-3，你受到{SelfDamage:diff()}点伤害，队友受到{TeamDamage:diff()}点伤害；<br>若结果为4-6，你获得{SelfStarLight:diff()}点星光，队友获得{TeamStarLight:diff()}点星光。 |
| `EventRedHeatWarning` | 红温警告 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>所有玩家获得{VigorPower:diff()}点活力。 |
| `EventsConcealingInvestigationA` | 隐匿调查·阶段1 | `0` | `技能` | `稀有` | `自己` | 隐匿调查<br>对随机非Boss非精英敌人施加1层标记。<br>邦尼与事件触发者获得隐匿。 |
| `EventsConcealingInvestigationB` | 隐匿调查·阶段2 | `0` | `技能` | `稀有` | `自己` | 隐匿调查<br>对随机非Boss敌人施加1层标记。<br>邦尼与事件触发者各获得1张攻击牌；若其抽牌堆与弃牌堆都没有攻击牌，则改为获得{energyPrefix:energyIcons(1)}。<br>然后双方获得隐匿。 |
| `EventsConcealingInvestigationC` | 隐匿调查·阶段3 | `0` | `技能` | `稀有` | `自己` | 隐匿调查<br>所有单位获得1层标记。<br>邦尼与事件触发者各获得1张攻击牌；若其抽牌堆与弃牌堆都没有攻击牌，则改为获得{energyPrefix:energyIcons(1)}。<br>然后双方获得隐匿。 |
| `EventsConcealingInvestigationD` | 真相揭露 | `0` | `技能` | `稀有` | `自己` | 隐匿调查<br>所有单位获得2层标记。<br>所有队友各获得1张攻击牌；若其抽牌堆与弃牌堆都没有攻击牌，则改为获得{energyPrefix:energyIcons(1)}。<br>然后所有队友获得隐匿。 |
| `EventSprint` | 疾跑 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>所有玩家获得{Dexterity:diff()}点临时敏捷。 |
| `EventThunderStrike` | 天打雷劈 | `0` | `技能` | `稀有` | `所有友方` | 事件<br>对所有单位造成{Damage:diff()}点伤害。 |
...
| `SkillRemoteIntrusion` | 远程侵入 | `0` | `技能` | `稀有` | `自己` | 立即获得1张攻击牌到手中，本回合其费用为0。<br>本回合取消被动防火墙弃牌效果。 |
...
| `SkillTroubleMaker` | 麻烦制造者 | `0` | `技能` | `稀有` | `自己` | 展示3张随机事件牌，选择1张立即发动。 |
...
| `CopyQuotaPower` | 复制计数 | `增益` | `计数` | 显示你本回合已打出多少张技能牌，最多显示到9。回合开始时重置为0。第10张及之后的技能牌不再触发【人格：忍者】的复制效果。 |
| `CosmosFreezesPower` | 静滞 | `减益` | `计数` | 此单位造成伤害时，会按(1-100/(100+层数))计算后直接减少伤害。<br>当前等效减力：{TemporaryStrengthLoss}。<br>当层数大于10且当前意图不是攻击时，每回合结束减少当前层数的10%，最低减少1层。 |
...
| `CyberKittyNodePower` | 节点 | `增益` | `计数` | 回合开始时，将节点值随机刷新为1到10。<br>数值越小，位置表现越向右；数值越大，位置表现越向左。<br>在Boss房中，不会触发这个位置表现效果。 |
...
| `ReversedScalesHolographicPower` | 幻影·逆鳞 | `增益` | `计数` | 每层使你受到敌方伤害时获得1点伤害减免。 |
| `ReversedScalesPower` | 幻影·逆鳞 | `增益` | `计数` | 每层使你受到敌方伤害时获得2点伤害减免。每次获得1层时，获得2点临时力量。回合结束时移除。 |
| `ShadowsLimitPower` | 暗影额度 | `增益` | `计数` | 显示本回合还能通过攻击牌施加几次双生之影。 |
| `StagnantCosmosPower` | 静滞协议 | `增益` | `计数` | 每当你打出一张实际消耗不低于{energyPrefix:energyIcons(1)}的攻击牌或技能牌并造成伤害时，消耗1层静滞协议，使目标获得等同于该牌费用+1的静滞。 |
...
| `TrueDragonFormPower` | 真龙形态 | `增益` | `不叠层` | 战斗开始时，已经升级且没有附魔的攻击牌获得华彩，并升级你的全部卡牌。<br>你的攻击牌伤害+4，获得2点力量、1点敏捷，并永久获得2点最大生命。不可叠加。 |
...
| `PersonCyberKitty` | 【人格：猫猫】 | `古代` | · 每3回合开始时，将1张远程侵入加入手中。<br>· 回合开始时，若你的节点大于3，获得1点临时力量和1点临时敏捷，将1张攻击牌置入手中；其费用-1，若费用大于3则改为-2，X费牌不变。<br>· 你每回合第一次打出攻击牌后，弃置手牌中最左侧的1张攻击牌，将其在本场战斗中升级。 |
| `PersonDeityLin` | 【人格：凛】 | `古代` | · 每3回合开始时，获得1层调查发现。<br>· 战斗开始时，将1张活体书页加入手中。<br>· 活体书页永久增伤为0~11时，每消耗1层人格衍生：活体书页，其基础伤害永久+1，且能量费用-1。<br>· 活体书页永久增伤为12~22时，每消耗2层后，其基础伤害永久+1。<br>· 活体书页永久增伤为23及以上时，每消耗3层后，其基础伤害永久+1。 |
...
| `PersonNinja` | 【人格：忍者】 | `古代` | · 每3回合开始时，将1张忍术连击加入手中。<br>· 回合开始时，复制计数重置为0。<br>· 每打出1张技能牌，复制计数+1，最多显示到9。<br>· 你每回合打出第3/6/9张技能牌时，复制最后打出的非人格技能牌加入手中，并附加消耗。<br>· 第10张及之后的技能牌不再触发此复制效果。 |
...
| `PersonShadowScion` | 【人格：阿尔】 | `古代` | · 每个地图阶段的第一场战斗开始时，所有玩家获得1张王国资产。<br>· 每3回合开始时，将1张强者怜悯加入手中。<br>· 当你使任意玩家获得卡牌或抽牌时，你与该玩家各获得1点星光。<br>· 只要获得星光，就会获得钞能力。<br>· 你每拥有13层永恒星光，战斗开始时便获得1点力量和1点敏捷。 |
| `PersonSlimeLulu` | 【人格：史莱姆】 | `古代` | · 最大生命值-10。<br>· 每3回合开始时，将1张治愈黏液加入手中。<br>· 受到伤害后，获得1层治愈，为伤害来源施加1层附着粘液，并使此人格遗物冷却-1。 |
| `PersonSocialFearNun` | 【人格：社恐修女】 | `古代` | · 每3回合开始时，将1张铁处女加入手中。<br>· 每回合开始时，施加1层边界强化。 |
| `PersonSupermanMegas` | 【人格：美甲师】 | `古代` | · 手牌上限+3；<br>· 每3回合开始时，将1张轨道轰炸加入手中。<br>· 回合结束时，若你的手牌少于5张，则下回合额外抽1张牌。<br>· 打出轨道轰炸后，抽1张牌。 |
...
| `PersonWeirdEgg` | 【人格：蒸蛋】 | `古代` | · 每3回合开始时，将1张麻烦制造者加入手中。<br>· 进入事件房间时，获得9金币。连续进入事件房间时，金币收益连续翻倍。<br>· 你触发任意事件卡时，获得3层星光。麻烦制造者除外。 |
...
| `PersonalityDerivativeLivingFolio` | 【人格衍生：活体书页】 | `古代` | · 进入事件房时，层数+1；<br>· 每次触发1张事件卡，层数+1；<br>· 打出活体书页需要消耗一层计数；若目标在打出前已有至少3层标记，则获得{energyPrefix:energyIcons(1)}。 |
...
| `TokenBlueBankCardGeneral` | 【银行卡·余额少】 | `普通` | 获得14层永恒星光。 |
...
| `TokenBlueSpeedRollerGeneral` | 【速度滑轮·初级】 | `普通` | 战斗开始时，获得1点敏捷。每攀爬12层，获得1次仅对下一层选择生效的。 |
...
| `TokenPurpleBankCardIntermediate` | 【银行卡·余额多】 | `非普通` | 获得28层永恒星光。 |
...
| `TokenPurpleMotorcycleHelmetIntermediate` | 【摩托头盔·中级】 | `非普通` | 战斗开始时，获得2层幻影·逆鳞。<br>每回合开始时，获得1点格挡。 |
...
| `TokenPurpleSpeedRollerIntermediate` | 【速度滑轮·中级】 | `非普通` | 战斗开始时，获得1点敏捷。每攀爬9层，获得1次仅对下一层选择生效的。 |
...
| `TokenGoldBankCardPremium` | 【银行卡·用不完】 | `稀有` | 获得14层永恒星光。<br>战斗开始时，所有玩家获得3点星光。 |
...
| `TokenGoldInitialPoint` | 【初始点】 | `稀有` | 在休息处，你可以花费金币升星<br>花费60/90/120金币，从3个筹码遗物中选择1个，恢复15%最大生命值；<br>完成3次后，该选项仅恢复15%最大生命值。<br>重复获取会改为三层永恒星光。 |
...
| `TokenGoldMotorcycleHelmetPremium` | 【摩托头盔·高级】 | `稀有` | 战斗开始时，获得2层幻影·逆鳞和1点敏捷；<br>每回合开始时，获得2点格挡。 |
...
| `TokenGoldSpeedRollerPremium` | 【速度滑轮·高级】 | `稀有` | 战斗开始时，获得2点敏捷。每攀爬6层，获得1次仅对下一层选择生效的。 |
...
| `TokenExclusiveCandyMembershipCard` | 【糖果会员证】 | `稀有` | 每回合开始时，如果你的手里没有怪奇糖果机，则添加1张怪奇糖果机到手中。<br>拥有【系列·幽魂暗巷】效果。 |
...
| `TokenExclusiveLittleSnakeDoll` | 【小蛇玩偶】 | `非普通` | 每次受到敌方伤害时，获得1层逆鳞。<br>拥有【系列·龙宫游乐园】效果。 |
...
| 内容类型 | 数量 | 说明 |
|------|------|------|
| 药水 | 8 | 包含起始人格宝箱选择药水、糖果系列药水和随机筹码包。 |
