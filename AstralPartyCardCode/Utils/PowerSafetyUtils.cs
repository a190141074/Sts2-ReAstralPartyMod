...
public static class PowerSafetyUtils
{
    ...
    public static bool IsSafePower(PowerModel power)
    {
        ...
        // 过滤掉已知有 Bug 的能力
        if (powerId.Contains("PERSONAL_HIVE") || powerId.Contains("SANDPIT")) return false;

        try
        {
            if (power.ShouldStopCombatFromEnding()) return false;
        }
        ...
        if (SafetyCache.TryGetValue(powerType, out var isSafe)) return isSafe;
    ...
    public static bool AnalyzePowerSafety(Type powerType)
    {
        try
        {
            ...
            {
                if (method.IsStatic || method.Name.StartsWith("get_") || method.Name.StartsWith("set_")) continue;

                if (MethodHasMonsterCast(method)) return false;

                if (MethodHasUnsafeDealerAccess(method)) return false;
                ...
                if (asyncAttr != null)
                {
                    ...
                    if (stateMachineType != null)
                    {
                        ...
                        {
                            if (MethodHasMonsterCast(moveNextMethod)) return false;
                            if (MethodHasUnsafeDealerAccess(moveNextMethod)) return false;
                        }
                    }
                }
            }
            ...
            foreach (var prop in properties)
            {
                ...
                {
                    if (MethodHasMonsterCast(getter)) return false;
                    if (MethodHasUnsafeDealerAccess(getter)) return false;
                }
            }
    ...
    public static bool MethodHasUnsafeDealerAccess(MethodInfo method)
    {
        try
        {
            ...
            if (dealerParamIndex == -1) return false;
            ...
            for (var i = 0; i < ilBytes.Length; i++)
            {
                ...
                if (opCode == 0x02 && dealerParamIndex == 0)
                    ...
                    hasNullCheck = CheckForNullCheckAfterLoad(ilBytes, i);
                else if (opCode == 0x05 && dealerParamIndex == 3) hasNullCheck = CheckForNullCheckAfterLoad(ilBytes, i);
    ...
    public static bool CheckForNullCheckAfterLoad(byte[] ilBytes, int loadIndex)
    {
        for (var i = loadIndex + 1; i < ilBytes.Length; i++)
        {
            ...
            if (opCode == 0x2C || opCode == 0x2D) return true;

            if (opCode == 0x39 || opCode == 0x3A) return true;

            if (opCode == 0x31) return true;
        }
