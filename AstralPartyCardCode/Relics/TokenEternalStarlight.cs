...
public class TokenEternalStarlight : AstralPartyRelicModel
{
    ...
    protected override string IconBasePath => "res://AstralPartyMod/images/relic/token_Eternal_Starlight";

    public static IHoverTip BuildReferenceHoverTip()
    {
        var title = new LocString("relics", "ASTRALPARTYMOD-TOKEN_ETERNAL_STARLIGHT.title");
        var body = new LocString("relics", "ASTRALPARTYMOD-TOKEN_ETERNAL_STARLIGHT.description")
            .GetRawText()
            .Replace("\n{CurrentSetLine}", string.Empty);
        return new HoverTip(
            title,
            body,
            GD.Load<Texture2D>("res://AstralPartyMod/images/relic/token_Eternal_Starlight.png")
        );
    }
    ...
    {
        var title = new LocString("relics", "ASTRALPARTYMOD-TOKEN_ETERNAL_STARLIGHT.current_set_title");
    ...
    private string GetCurrentSetLineText()
    {
        return GetCurrentSetLine()?.GetRawText()
               ?? new LocString("relics", "ASTRALPARTYMOD-TOKEN_ETERNAL_STARLIGHT.current_set_none").GetRawText();
    }
